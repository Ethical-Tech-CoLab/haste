...
HTTP API → Azure Storage Queue → Azure Function (trigger)
                            ...
                            runs Docker container task
                               ...
                               ┌────────┴────────┐
                               │  IN_PROGRESS?   │
                               │  Re-queue self  │← function exits, queue message
                               └────────┬────────┘  re-appears after visibility timeout
                                        │
                                   COMPLETED / FAILED
                                        │
                                  Save metadata,
...
- **Post-run (separate error scope)** → queues artifact zipping regardless of success/failure, as long as an output path exists
